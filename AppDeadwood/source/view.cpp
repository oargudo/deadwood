...
void View::projectingRay(int sx, int sy, vpPoint & start, Vector & dirn)
{
    ...
    realy = int(height + 2.0f * starty - sy); realx = sx;
...
void View::projectMove(int ox, int oy, int nx, int ny, vpPoint cp, Vector & del)
{
    ...
    // unproject new point
    // unproject screen point to derive world coordinates
    realy = int(height + 2.0f * starty - ny); realx = nx;
    win = glm::vec3((float) realx, (float) realy, 0.5f);
    wrld = glm::unProject(win, getViewMtx(), getProjMtx(), viewport);
    npnt = vpPoint(wrld.x, wrld.y, wrld.z);

    // unproject old point
    realy = int(height + 2.0f * starty - oy); realx = ox;
    win = glm::vec3((float) realx, (float) realy, 0.5f);
    wrld = glm::unProject(win, getViewMtx(), getProjMtx(), viewport);
...
void View::varietyview()
{
    ...
    zoomdist = 3.8f * viewscale;

    a[0] = 0.0f; a[1] = 1.0; a[2] = 0.0;
    axis_to_quat(a, -7.0f * float(PI2)/8.0f, curquat); // -1.0 for join-manip
...
bool View::spin()
{
    ...
    {
        a[0] = 0.0f; a[1] = 1.0; a[2] = 0.0;
        axis_to_quat(a, (spinsteps - (float) focalstep) * float(PI2)/spinsteps, curquat);
 ...
 */
void
ecosim::trackball(float q[4], float p1x, float p1y, float p2x, float p2y)
{
 ...
 */
void
ecosim::axis_to_quat(float a[3], float phi, float q[4])
{
    vnormal(a);
    vcopy(a,q);
