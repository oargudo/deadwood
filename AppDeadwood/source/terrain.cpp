...
float Terrain::getHeightAtCoords(float x, float y, bool interp) const
{
    float convx = (float)(grid->width() - 1) / dimx;
    float convy = (float)(grid->height() - 1) / dimy;
    float i = x * convx;
    float j = y * convy;
    if (!interp) return grid->get(int(i), int(j));
    else return grid->getBilinear(i, j);
}

float Terrain::getFlatHeight(int idx) const
{
    int x, y, dx, dy;
    getGridDim(dx, dy);
...
void Terrain::getNormal(int x, int y, Vector & norm) const
{
    vpPoint x1, x2, y1, y2;
    int dx, dy;
    Vector dfdx, dfdy;
...
vpPoint2D Terrain::getGradient(int x, int y) const
{
    int dx, dy;
    getGridDim(dx, dy);
    ...
    dfdx.diff(x1, x2);
    dfdy.diff(y1, y2);
    return vpPoint2D(dfdx.j / dfdx.i, dfdy.j / dfdy.k);
}
...
bool Terrain::loadElv(const uts::string &filename)
{
    float lat;
    int dx, dy;
