...
void WaterFill::compute(uint step)
{
    ...
    for(uint x = 0; x< dx; ++x)
        for(uint y = 0; y< dy; ++y)
        {
            ...
            for(int j =0; j<8; ++j)
            {
                ...
                if(this_h > ch)
                {
                    double slope = (this_h - ch) / (j < 4 ? 1.0 : sqrt(2.0));
                    slopes[numLower] = slope;
                    neighs[numLower] = cur;
                    sumSlopes += slope;
                    numLower++;
                }
            }
            // choose randomly one of the lower neighs, using as prob the step
            double rr = sumSlopes * (dice.generate() / 1000.0f);
            double accum = 0.0;
            int ri = 0;
            while (ri < numLower) {
                accum += slopes[ri];
    ...
    // add all passes toward the boundary
