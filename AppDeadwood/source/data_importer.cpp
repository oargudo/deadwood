...
void data_importer::modelset::setup_ranges()
{
    ...
    binsize = minrange;
    nbins = int(std::ceil(ranges.back() - ranges.front()) / binsize);
...
int data_importer::modelset::sample_selection_fast(float height)
{
    ...
    int idx = int(height - ranges.front());
    if (idx >= samplemap.size() || height > ranges.back())
    {
        throw std::runtime_error("height out of range in modelset::sample_selection");
    }
    ...
    if(infile.is_open())
    {
        ...
        for(int c = 0; c < numcat; c++)
        {
            ...
            for (int plnt_idx = 0; plnt_idx < nplants; plnt_idx++)
            {
                ...
                infile >> height;
                infile >> radius;
                MinimalPlant plnt = {x, y, height, radius, z, false, cat};
...
static species_params parse_species_line(std::string line, std::string name)
{
    ...
    for (; sstr.good(); i++)
    {
        ...
        {
        case (0):
            location = std::stof(token);
            locs.push_back(location);
            break;
        case (1):
            width_scale = std::stof(token);
...
static int sql_callback_common_data_species(void *write_info, int argc, char ** argv, char **colnames)
{
    ...
    float trunkrad, creg, mor, wetbiomass, wooddensity, logseeding, hghtdense, hghtisolate, raddense, radisolate, qval;
    bool istree, isconifer, isherb;
    ...
    sp.qval = qval;
    std::cerr << sp.idx << ": creg = " << sp.creg << " mor = " << sp.mor << " wetbiomass = " << sp.wetbiomass << " density = " << sp.wooddensity;
    std::cerr << " hghtdense = " << sp.hghtdense << " hghtisolate = " << sp.hghtisolate << " raddense = " << sp.raddense << " radisolate = " << sp.radisolate << " qval = " << sp.qval << std::endl;
...
static int sql_callback_common_data_all_species(void *write_info, int argc, char ** argv, char **colnames)
{
    ...
    sp.a = a;
    sp.b = b;
    sp.maxage = int(maxage);
    sp.maxdeadage = int(maxdeadage);
