...
bool PlantGrid::readVersionedPDB(string filename, Biome * biome, Terrain * ter, float &maxtree, bool live)
{
    ifstream infile;
    char s;
    ...
    if(infile.is_open())
    {
        infile >> s; // check version
        infile.close();
        if(s != 'v') // prior to versioning
            return readPDBv1(filename, biome, ter, maxtree);
        else // may need finer discernment of version later
        {
            char n, d, r;
            infile >> n >> d >> r; // pattern is '2.r';
            if(r == '1')
            {
                cerr << "PDB v2.1" << endl;
                return readPDBv2r1(filename, biome, ter, maxtree, live);
            }
            else if (r == '2')
            {
                cerr << "PDB v2.2" << endl;
                return readPDBv2r2(filename, biome, ter, maxtree, live);
            }
            else {
                cerr << "PDB v2.3" << endl;
                return readPDBv2r3(filename, biome, ter, maxtree, live);
            }
        }
    }
    else
    {
        cerr << "Error Mesh::readVersionedPDB: unable to open " << filename << endl;
        return false;
    }
    return true;
}

bool PlantGrid::readPDBv1(string filename, Biome * biome, Terrain * ter, float &maxtree)
{
    ifstream infile;
    float rndoff;
...
bool PlantGrid::readPDBv2r1(string filename, Biome * biome, Terrain * ter, float &maxtree, bool live)
{
    ifstream infile;
    std::string version, discard;
    float rndoff;
    int liveNum, deadNum, logNum, numSpecies, activeSpecies;

    cerr << "PLANT GRID READ OF " << filename << endl;
    infile.open((char *) filename.c_str(), ios_base::in);
    if(infile.is_open())
    {
        infile >> version;
        infile >> numSpecies;
        cerr << "numspecies = " << numSpecies << endl;

        for(int k = 0; k < numSpecies; k++)
        {
            long id, pid;
            int n, c, a, e, num;
            float x, y, z, h, r, v, s, d;
            Plant p;

            cerr << "Iteration " << k << endl;
            infile >> activeSpecies >> discard;
            cerr << activeSpecies << " " << discard << endl;
            infile >> liveNum >> deadNum >> logNum;

            cerr << "species = " << activeSpecies << " live = " << liveNum << " dead = " << deadNum << " logs = " << logNum << endl;

            for(n = 0; n < liveNum; n++)
            {
                infile >> id >> pid >> a >> x >> y >> z >> h >> r >> v;
                // cerr << id << " " << x << " " << y << " " << z << endl;

                // discard remaining life history data
                infile >> num;

                if(live && a == num) // vigour history matchers age so tree hasn't yet died
                {
                    p.height = h;
                    if(h > maxtree)
                        maxtree = h;
                    ...
                    p.pos = vpPoint(y, z, x);
                    placePlant(ter, activeSpecies, p);
                ...
                for(int i = 0; i < num; i++) // vigour history
                    infile >> v >> h >> r;

                infile >> num;
                for(int i = 0; i < num; i++) // event history
                    infile >> c >> a >> s;

                infile >> num;
                for(int i = 0; i < num; i++) // drop history
                    infile >> c >> e >> a >> pid;
            }
            ...
            for(n = 0; n < deadNum; n++)
            {
                float decay;
                int dc;
                infile >> id >> pid >> a >> x >> y >> z >> h >> r >> decay;
                ...
                if(!live)
                {
                    ...
                    if(decay > def_snagDC1)
                    {
                        dc = 1;
                    }
                    else
                    {
                        if(decay > def_snagDC2)
                        {
                            dc = 2;
                        }
                        else
                        {
                            if(decay > def_snagDC3)
                            {
                                dc = 3;
                            }
                            else
                    ...
                    // use decay class as a basis for colouring and adjustment of canopy radius
                    // note that for snags the colour is absolute and not a species offset
                    switch(dc)
                    {
                    case 1:
                        // p.col = glm::vec4(-1.0f, -1.0f, -1.0f, 1.0f);
                        p.col = glm::vec4(-0.5f, -0.5f, -0.5f, 1.0f);
                        break;
                    case 2:
                        // p.col = glm::vec4(-0.75f, -0.75f, -0.75f, 1.0f);
                        p.col = glm::vec4(-0.65f, -0.65f, -0.65f, 1.0f);
                        //p.canopy *= 0.5f;
                        break;
                    case 3:
                        p.col = glm::vec4(-0.8f, -0.8f, -0.8f, 1.0f);
                        //p.canopy *= 0.2f;
                        break;
                    case 4:
                        p.col = glm::vec4(-0.9f, -0.9f, -0.9f, 1.0f);
                        // p.canopy *= 0.1f;
                        break;
                    case 5:
                        // p.canopy *= 0.05f;
                ...
                // discard decay, event and drop history
                infile >> num;
                for(int i = 0; i < num; i++) // decay history
                    infile >> d;
            ...
            // read and discard logs
            for(n = 0; n < logNum; n++)
            {
                float decay, dirn, len;

                infile >> id >> pid >> a >> decay >> dirn >> len;
            }
        }
    ...
    {
        cerr << "Error Mesh::readPDBv2r1: unable to open " << filename << endl;
        return false;
    }
    return true;
}

bool PlantGrid::readPDBv2r2(string filename, Biome * biome, Terrain * ter, float &maxtree, bool live)
{
    ifstream infile;
    std::string version, discard;
    float rndoff;
    ...
    cerr << "TERSE PLANT GRID READ OF " << filename << endl;
    infile.open((char *) filename.c_str(), ios_base::in);
    if(infile.is_open())
    {
        infile >> version;
        ...
        for(int k = 0; k < numSpecies; k++)
        {
            ...
            for(n = 0; n < liveNum; n++)
            {
                ...
                if(live) // vigour history matchers age so tree hasn't yet died
                {
                    p.height = h;
                    if(h > maxtree)
                        maxtree = h;
            ...
            for(n = 0; n < deadNum; n++)
            {
                ...
                infile >> id >> pid >> a >> x >> y >> z >> h >> r >> decay >> lvh;
                ...
                // discard decay and drop history
                infile >> num;
                for(int i = 0; i < num; i++) // event history
                {
                    infile >> c >> a >> s;
            ...
            for(n = 0; n < logNum; n++)
            {
                ...
                infile >> id >> pid >> a >> decay >> dirn >> len >> x >> y >> z;
            }
        }
    ...
    {
        cerr << "Error Mesh::readPDBv2r2: unable to open " << filename << endl;
        return false;
    }
    return true;
}
...
bool PlantGrid::readPDBv2r3(string filename, Biome * biome, Terrain * ter, float &maxtree, bool live)
{
    ifstream infile;
    std::string version, discard;
    float rndoff;
    ...
    if(infile.is_open())
    {
        ...
        for(int k = 0; k < numSpecies; k++)
        {
            ...
            for(n = 0; n < liveNum; n++)
            {
                infile >> id >> pid >> a >> x >> y >> z >> h >> r >> v >> dbh;
                // cerr << id << " " << x << " " << y << " " << z << endl;
            ...
            for(n = 0; n < logNum; n++)
            {
                float decay, dirn, len, diam;
                int cat;

                infile >> id >> pid >> a >> decay >> dirn >> len >> diam >> x >> y >> z >> cat;
            }
        }
    ...
    {
        cerr << "Error Mesh::readPDBv2r3: unable to open " << filename << endl;
        return false;
    }
    return true;
}
...
bool PlantGrid::writePDBv1(string filename, Biome * biome)
{
    ofstream outfile;
    ...
    {
        cerr << "Error Mesh::writePDBv1: unable to open " << filename << endl;
        return false;
    }
...
//    // canopy - cone
//    idt = glm::mat4(1.0f);
//    trs = glm::vec3(0.0f, 0.9f, 0.0f);
//    tfm = glm::translate(idt, trs);
//    tfm = glm::rotate(tfm, glm::radians(-270.0f), rotx);
//    //tfm = glm::translate(tfm, glm::vec3(0.0f, 0.0f, -canopyheight));
//#ifdef HIGHRES
//    shape.genCappedCone(0.5f, canopyheight-0.1f, 20, 1, tfm, false);
//#endif
//#ifdef LOWRES
//    shape.genCappedCone(0.5f, canopyheight-0.1f, 6, 1, tfm, false);
//#endif
//
//    // with sphere as cap to round the top
//    idt = glm::mat4(1.0f);
//    trs = glm::vec3(0.0f, 0.9f, 0.0f);
//    tfm = glm::translate(idt, trs);
//    tfm = glm::scale(tfm, glm::vec3(1.0, 0.1f, 1.0f)); // make sure tree fills 1.0f on a side bounding box
//    tfm = glm::rotate(tfm, glm::radians(90.0f), rotx);
//
//#ifdef HIGHRES
...
//            for(s = 0; s < 2 * biome->numPFTypes(); s++)
//            {
//                Shape shape;
...
//
//        // and dead version of plant
//        GLfloat black[4] = {1.0f, 1.0f, 1.0f, 1.0f};
//        currshape.setFlatColour(black);
//        shapes[0][s+biome->numPFTypes()] = currshape;
//    }
//}
//
...
//    std::vector<std::vector<glm::vec4> > colvars; // colour variation for each instance
//
//    xforms.resize(2 * biome->numPFTypes());
//    colvars.resize(2 * biome->numPFTypes());
//
//    for(x = sx; x <= ex; x++)
//        for(y = sy; y <= ey; y++)
//        {
...
//
//            for(s = 0; s < (int) plnts->pop.size(); s++)
//            {
//                std::vector<glm::mat4> xform, dxform; // transformation to be applied to each instance
//                std::vector<glm::vec4> colvar, dcolvar; // colour variation to be applied to each instance
//                if((* plantvis)[s])
//                {
...
//
//
//                            if(plnts->pop[s][p].living)
//                            {
//                                xform.push_back(tfm);
//                                colvar.push_back(plnts->pop[s][p].col); // pass in colour variation
//                                // cerr << plnts->pop[s][p].col[0] << endl;
//                            }
//                            else
//                            {
//                                dxform.push_back(tfm);
//                                dcolvar.push_back(plnts->pop[s][p].col);
//                                // dcolvar.push_back(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
//                            }
//                            bndplants++;
//                        }
//                        else
...
//                xforms[s+biome->numPFTypes()].insert(xforms[s+biome->numPFTypes()].end(), dxform.begin(), dxform.end());
//                colvars[s+biome->numPFTypes()].insert(colvars[s+biome->numPFTypes()].end(), dcolvar.begin(), dcolvar.end());
//                f = flatten(x, y);
...
void EcoSystem::init()
{
    esys = PlantGrid(pgdim, pgdim, biome->numPFTypes());
    //eshapes = ShapeGrid(pgdim, pgdim, biome);
    // cmap = ConditionsMap();

    for(int i = 0; i < maxNiches; i++)
    {
        PlantGrid pgrid(pgdim, pgdim, biome->numPFTypes());
...
void EcoSystem::clear()
{
    ...
    for(int i = 0; i < (int) niches.size(); i++)
    {
        niches[i] = PlantGrid(pgdim, pgdim, biome->numPFTypes());
    }
}

bool EcoSystem::loadNichePDB(string filename, Terrain * ter, int niche, bool live)
{  
    bool success;

    // std::cout << "Number of niches: " << niches.size() << std::endl;
    success = niches[niche].readVersionedPDB(filename, biome, ter, maxtreehght, live);
    if(success)
    {
...
{
    return niches[niche].writePDBv1(filename, biome);
}
