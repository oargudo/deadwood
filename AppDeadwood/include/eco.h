...
#define def_snagDC1 0.9f        // transition thresholds between snag decay classes
#define def_snagDC2 0.67f
#define def_snagDC3 0.5f
#define def_snagDC4 0.33f
...
struct Plant
{
    ...
    bool living;    //< whether the plant is living (true) or a snag (false)
...
public:
    ...
    int numSpecies; //< number of plant species

    PlantGrid(){ gx = 0; gy = 0; numSpecies = 0; initGrid(); initSpeciesTable(); }

    PlantGrid(int dx, int dy, int numS){ gx = dx; gy = dy; numSpecies = numS; initGrid(); initSpeciesTable(); }
    ...
    /**
     * Read in plant positions from a PDB format text file, calling submethod as appropriate to the version
     * @param filename  name of file to load (PDB format)
     * @param biome biome to which the plants belong
     * @param ter  terrain onto which trees are projected
     * @param maxtree   update height if it exceeds the currently tallest tree
     * @param live      if true load only live trees, otherwise load only dead trees
     * @retval true  if load succeeds,
     * @retval false otherwise.
     */
    bool readVersionedPDB(string filename, Biome * biome, Terrain * ter, float & maxtree, bool live = true);

    /**
     * Read in plant positions from a version1 PDB format text file
     * @param filename  name of file to load (PDB format)
     * @param biome biome to which the plants belong
     * @param ter  terrain onto which trees are projected
     * @param maxtree   update height if it exceeds the currently tallest tree
     ...
     * @retval false otherwise.
     */
    bool readPDBv1(string filename, Biome * biome, Terrain * ter, float & maxtree);

    /**
     * Read in plant positions from a version 2.1 PDB format text file
     * @param filename  name of file to load (PDB format)
     * @param biome biome to which the plants belong
     * @param ter  terrain onto which trees are projected
     * @param maxtree   update height if it exceeds the currently tallest tree
     ...
     * @retval false otherwise.
     */
    bool readPDBv2r1(string filename, Biome * biome, Terrain * ter, float & maxtree, bool live = true);
    bool readPDBv2r2(string filename, Biome * biome, Terrain * ter, float & maxtree, bool live = true);
    bool readPDBv2r3(string filename, Biome * biome, Terrain * ter, float & maxtree, bool live = true);

    /**
     * Write plant positions to a version 1 PDB format text file
     * @param filename  name of file to load (PDB format)
     * @param biome     biome to which the plants belong
     * @retval true  if load succeeds,
     * @retval false otherwise.
     */
    bool writePDBv1(string filename, Biome * biome);

    /**
     * Write plant positions to a version 0 PDB format text file. Simple list of plant positions.
     * @param filename  name of file to load (PDB format)
     * @param biome     biome to which the plants belong
     * @retval true  if load succeeds,
     * @retval false otherwise.
     */
    bool writePDBv0(string filename, Biome * biome);
};
...
//     */
//    void attachBiome(Biome * shpbiome){ biome = shpbiome; initGrid(); } // genPlants(); }
//
//    /**
...
public:
    ...
    void setBiome(Biome * ecobiome){
        biome = ecobiome;
        clear();
        //eshapes.attachBiome(ecobiome);
    }
    ...
    /**
       ...
       * @param live      which tree class to load (live if true, dead if false). Applies to v2.0 onwards
       * @retval true  if load succeeds,
       * @retval false otherwise.
       */
    bool loadNichePDB(string filename, Terrain * ter, int niche = 0, bool live = true);
