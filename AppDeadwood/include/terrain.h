...
namespace ecosim {
    ...
    public:
        ...
        float* getGridPtr() { return grid->getPtr(); }
        ...
        float getHeightRange() const { return hghtrange; }
        ...
        float getHeightAtCoords(float x, float y, bool interp = false) const;

        /// Obtain height at a flattened grid index
        float getFlatHeight(int idx) const;

        /// Return a world-space normal given a grid position
        void getNormal(int x, int y, Vector& norm) const;
        vpPoint2D getGradient(int x, int y) const;
        float getAspect(int x, int y) const;
        ...
        void getGridDim(unsigned int& dx, unsigned int& dy) const
        {
            dx = (unsigned int)grid->width();
            dy = (unsigned int)grid->height();
        }
        ...
        /**
           ...
           */
        bool loadElv(const uts::string& filename);
