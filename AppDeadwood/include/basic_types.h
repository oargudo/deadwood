...
struct output_tree : basic_tree
{
    output_tree(float x, float y, float z, float radius, float height, float dbh)
        : basic_tree(x, y, radius, height, dbh), z(int(z))
    {}
...
public:
    ...
    float getBilinear(float x, float y) const
    {
        ...
        float v   = fx*v1  + (1 - fx)*v0;
        return v;

    }


    void gaussianBlur(float sigma, int radius = -1) {
        if (radius <= 0)
        ...
        // Temporary buffer
        std::vector<float> temp(fmap.size());
        ...
        for (int y = 0; y < gy; ++y) {
            for (int x = 0; x < gx; ++x) {
                ...
                for (int k = -radius; k <= radius; ++k) {
                    int xx = std::clamp(x + k, 0, gx - 1);
                    blurred += get(xx, y) * kernel[k + radius];
                }
                temp[y * gx + x] = blurred;
            }
        }
        ...
        for (int y = 0; y < gy; ++y) {
            for (int x = 0; x < gx; ++x) {
                float blurred = 0.0f;
                for (int k = -radius; k <= radius; ++k) {
                    int yy = std::clamp(y + k, 0, gy - 1);
                    blurred += temp[yy * gx + x] * kernel[k + radius];
                }
                set(x, y, blurred);
            }
        }
...
public:
    ...
    /// set grass heights to a uniform value
    void fill(float h){ fmap.clear(); fmap.resize(gx*gy, int(h)); }
